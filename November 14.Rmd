...
```{r}
episodes_r1<- add.stamm(episodes_r1,stamm.up)
episodes_r2<- add.stamm(episodes_r2,stamm.up)
#episodes_r3<- add.stamm(episodes_r3,stamm.up)
episodes_r4<- add.stamm(episodes_r4,stamm.up)
...
dataset.addition<- function(episodedf, df){
  patients.episodedf<- levels(as.factor(as.character(episodedf$uniPatID)))
  preselector<- as.character(adddata$uniPatID) %in% patients.episodedf
  adddata<- df[preselector,]
  episodedf<- episodedf|>
    arrange(uniPatID,start_date)
...
decision.rules<- c(1,2,4)
icd10.codes.3<- c()
for(i in decision.rules){
  im<- get(paste0("full.data.episodes_r",i))
  im<- as.vector(im[,grepl("icd10",colnames(im))])
  print(str(im))
