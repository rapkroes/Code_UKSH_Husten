...
episodes<- episode.fun.alt(ipc.up, 56)
```
...
# no. 32
discovery.tool<- function(decision.rule, agelims=c(0,150), insurance=c("all","GKV","PKV")[1], remove.copd=FALSE, lims.chronic.diseases=c(0,100), pandemic="none", preliminary.diag=c(NA,TRUE,FALSE)[1], AUB=c(NA,TRUE,FALSE)[1], quarter.selection=1:4, month.selection=1:12, dependent.variable, research.question){
  ...
  if(all(agelims!=c(0,150))){
    ...
    age.selector[is.na(age.selector)]<- FALSE
  }
  ...
  if(dependent.variable=="diag"){
    ...
    d$y<- age.weights(min(d$x),max.age)
...
escalation.parameters_lab<- list(type=2)
