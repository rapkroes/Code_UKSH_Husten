...
add.stamm.new.par<- function(edf,sdf, no.splits,no.workers){
  #paralellised version of add.stamm.new
  ...
  out<- as.data.frame(matrix(NA, nrow = nrow(edf), ncol = length(c(colnames(edf),colnames(sdf),"stamm.is.there"))))
  colnames(out)<- c(colnames(edf),colnames(sdf),"stamm.is.there")
  ticker<- 1
...
add.lab.par<- function(episodedf, lab, no.splits, no.workers){
  ...
  result<- parLapply(cl=par.cl, 1:no.splits,function(k){
    edf<- dl_e[[k]] |>
      arrange(uniPatID)
    labdf<- dl_lab[[k]]
    out<- matrix(NA,nrow = nrow(edf), ncol = 6)
    ...
    if(nrow(im_lab)>0){
      for(i in seq(1, nrow(im_lab))){
        if (im_lab$TG_DateNum[i]>=edf$start_date[1] && im_lab$TG_DateNum[i]<=edf$end_date[1]) {
          selector<- max(which(im_lab[i,]==1)) #max because we want to select the last column
    ...
    colnames(out)<- c(colnames(edf),colnames(labdf)[(nrow(labdf)-5):nrow(labdf)])
    return(out)
  })
  out<- as.data.frame(matrix(NA,nrow = nrow(episodedf), ncol = ncol(episodedf)+6))
...
episodes_r1.2<- add.stamm.new(episodes_r1,stamm.up)
episodes_r2.2<- add.stamm.new(episodes_r2,stamm.up)
