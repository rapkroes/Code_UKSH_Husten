...
#decision rule 4: If the timing of cases overlaps, we check whether there is a change in quarter between those two visits. If there is a change, we treat them as one, otherwise we set the last day of the earlier case to be last day of the quarter.
cases.dr1<- function(casedf){
  n<- nrow(casedf)
  out<- casedf
  for(i in seq(2,n)){
...
#add end date as min() mit na.rm=TRUE

cases.dr3<- function(casedf,ipc, debug=FALSE){
...
cases.dr4<- function(casedf){
  ...
  earliest.POSIXlt<- TG_DateNum2date(earliest.TG_Date)
  latest.POSIXlt<- TG_DateNum2date(latest.TG_Date)
